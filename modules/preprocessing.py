...
class Preprocessor:
    ...
    def __init__(self,project_dir,k_split=False):
    ...
    def data_split(self,output_feature):
        ...
        if self.k_split:
            ...
            self.X_train, self.X_valid, self.y_train, self.y_valid = train_test_split(X, y, random_state=42, test_size=0.33)
        self.X_test = pd.read_csv(self.test_file_path)
        id_cols = self.X_test['id']
        print("Split data into test, train and valid\n")
        if self.k_split:
            return self.X_train, self.y_train, self.X_test, id_cols
        else:
            return self.X_train, self.y_train, self.X_valid, self.y_valid, self.X_test, id_cols
